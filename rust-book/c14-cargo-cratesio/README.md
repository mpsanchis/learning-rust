...
A parent `Cargo.toml` contains a `[workspace]` section (instead of a `[package]` section), where it lists all the (sub-)packages in it. Sub-packages can be created with `cargo new <binary-crate-name>` or `cargo new <library-crate-name> --lib`.
...
WIP: continue at https://doc.rust-lang.org/book/ch14-03-cargo-workspaces.html#creating-the-second-package-in-the-workspace